...
def ADD_STUDENT(request):
    global email, username, first_name, last_name, profile_pic, password, course_id, session_year_id, address, gender
...
def STUDENT_ATTENDANCE(request):
    return render(request,'Hod/student_attendance.html')
...
@login_required(login_url='/')
def STAFF_SEND_NOTIFICCATION(request):
    staff = Staff.objects.all()
...
def STAFF_FEEDBACK_SAVE(request):
    if request.method=="POST":
        ...
        feedback.status = 1
        feedback.save()
        messages.success(request, 'Reply Are Successfully Sent !')
        return redirect('staff_feedback_reply')

@login_required(login_url='/')
def STUDENT_SEND_NOTIFICCATION(request):
    student = Student.objects.all()
...
@login_required(login_url='/')
def SAVE_STUDENT_NOTIFICCATION(request):
    if request.method == "POST":
        message = request.POST.get('message')
        student_id = request.POST.get('student_id')
        student = Student.objects.get(admin=student_id)
